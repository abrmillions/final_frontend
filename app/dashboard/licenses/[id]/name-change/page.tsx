...
export default function NameChangePage() {
  ...
  const handleSubmit = async () => {
    ...
    if (!newName.trim()) {
      toast({ title: "Missing", description: "Enter a new company name", variant: "destructive" })
      return
    }
    try {
      setSubmitting(true)
      const payload: any = {
        ...
        data: { subtype: "company_name_change", companyName: newName.trim() },
      }
      const client = await import('@/lib/api/django-client')
      ...
      toast({ title: "Submitted", description: "Your name change request was submitted" })
    } catch (e: any) {
      ...
      {!submittedAppId ? (
        <>
          <div className="flex items-center justify-between mb-6">
            <h1 className="text-2xl font-bold">Company Name Change</h1>
          ...
          <Card className="max-w-xl">
            <CardHeader>
              <CardTitle>New Company Name</CardTitle>
              <CardDescription>Enter the new registered company name</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="newName">New Name</Label>
                <Input id="newName" value={newName} onChange={(e) => setNewName(e.target.value)} disabled={loading} />
              </div>
        ...
        <>
          ...
          <Card className="max-w-xl">
            <CardHeader>
              <CardTitle>Name Change Submitted</CardTitle>
              <CardDescription>Your application was created successfully</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="flex gap-3">
                <Button asChild className="flex-1"><Link href={`/dashboard/applications?newApp=name-change&appId=${encodeURIComponent(submittedAppId || '')}`}>View Applications</Link></Button>
