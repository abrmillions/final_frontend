...
import { Alert, AlertDescription, AlertTitle } from "@/components/ui/alert";
 ...
 type PracticingReplacementForm = {
   licenseNumber: string;
   lossEvidence?: File | null;
 };
 
 export default function PracticingReplacementPage() {
  ...
  const disabled = maintenanceMode && !isAdmin;
   ...
   const [form, setForm] = useState<PracticingReplacementForm>({
     licenseNumber: "",
     lossEvidence: null,
   });
    ...
    if (disabled) {
     ...
     try {
       const payload: any = {
         ...
         subtype: "professional_practicing_replacement",
         data: {
       ...
       toast({ title: "Practicing replacement submitted", description: "" });
       router.push("/dashboard/applications");
     } catch (err: any) {
       setError(err?.message || "Failed to submit practicing replacement");
     } finally {
       setSubmitting(false);
     }
   };
   ...
   return (
     <div className="min-h-screen bg-background">
       <div className="container mx-auto px-4 py-8 max-w-3xl">
         <Card>
           <CardHeader>
             <CardTitle>Replacement of Practicing Professional License</CardTitle>
             <CardDescription>Re-issue a practicing license card/certificate.</CardDescription>
           </CardHeader>
           <CardContent>
            {disabled && (
             ...
             <form onSubmit={handleSubmit} className="space-y-4">
               ...
               <div className="pt-4">
                <Button type="submit" disabled={submitting || disabled}>
                   {submitting ? "Submitting..." : "Submit Practicing Replacement"}
                 </Button>
               </div>
             </form>
           </CardContent>
